...
//
// functions/api/_reconcile.js (underscore = not routed by Pages, same
// convention as _presend.js / _github-oidc.js). Ported from branch
// claude/seed-rehearsal-inputs for the monday rehearsal. There is NO routed
// entry point: the only caller is rehearsal.js, in-process, and it never puts
// a subject, detail or ref from here into a response, a log line or a digest.
//
// ─── CHANGES IN THE REHEARSAL PORT ───────────────────────────────────────────
//   • fetchImpl is REQUIRED. Every reader takes opts.fetchImpl and nothing
//     else: a missing one is unreadable "no-fetch", never the global. The
//     rehearsal passes its GET-only Stripe lock.
//   • has_more must be a boolean on every page, or the read is "bad-shape".
//   • Churn coverage is judged on the endpoint at the EXACT path
//     WEBHOOK_PATH on siteHost; endpoints on the host at another path make it
//     webhook_endpoint_path_ambiguous, not covered.
//   • Two or more Stripe customers entitled under one email is a finding
//     (duplicate_stripe_customers_same_email).
//   • The invariant stripe_entitled_total === considered + classified_other
//     is checked; a break is invariant_broken.
//   • Every finding and notice carries a PRIVATE ref: {row, sub}: row is the
//     index into the roster array passed in (or null), sub the index into
//     stripeResult.subs (or null). maskEmail keeps the domain and cannot tell
//     two buyers on one mail provider apart; ref can. The de-duplication key
//     includes ref for the same reason.
//   • Three more GET-only readers for C8: listStripePrices,
//     listStripePromotionCodes, listStripePaymentLinks.
//
...
const STRIPE_PROMOS_URL = "https://api.stripe.com/v1/promotion_codes";
const STRIPE_LINKS_URL = "https://api.stripe.com/v1/payment_links";
// The path stripe-webhook.js is served at. Churn coverage is judged on the
// enabled endpoint(s) at exactly this path on siteHost.
export const WEBHOOK_PATH = "/api/stripe-webhook";
const MAX_LINE_ITEM_READS = 10;
...
export async function listStripeSubscriptions(opts = {}) {
  // REQUIRED. No global fallback: a caller that forgets it gets "no-fetch".
...
// never an event body. The URL is kept as host and path only (no query).
//
...
export async function listStripeWebhookEndpoints(opts = {}) {
  const r = await listAll(opts, STRIPE_WH_URL, [], "no Stripe read key configured");
  if (!r.readable) return { ...r, endpoints: [], items: [] };
  const endpoints = r.items.map((e) => ({
    url_host: hostOf(e && e.url),
    url_path: pathOf(e && e.url),
  ...
  return { ...r, endpoints, items: endpoints };
}

// ── the three C8 readers (rehearsal port) ────────────────────────────────────
// Same contract as the readers above: GET only, through the REQUIRED
// fetchImpl, pinned Stripe-Version, has_more must be a boolean, paged on
// starting_after up to MAX_PAGES, never throwing, returning
// {readable, items, status, reason, requestId, pages, detail}.

// GET /v1/prices?active=true&limit=100. Items keep only what C8 compares.
export async function listStripePrices(opts = {}) {
  const r = await listAll(opts, STRIPE_PRICES_URL, [["active", "true"]], "no Stripe read key configured");
  if (!r.readable) return r;
  return { ...r, items: r.items.map((p) => ({
    id: String((p && p.id) || ""),
    product: typeof (p && p.product) === "string" ? p.product : String((p && p.product && p.product.id) || ""),
    unit_amount: typeof (p && p.unit_amount) === "number" ? p.unit_amount : null,
    currency: String((p && p.currency) || "").toLowerCase(),
    active: !!(p && p.active),
    recurring: !!(p && p.recurring),
  })) };
}

// GET /v1/promotion_codes?active=true&limit=100. On the pinned API version a
// promotion code carries its coupon under promotion.coupon (an expandable id);
// older versions carried it as code.coupon. The coupon's applies_to is only
// returned when expanded. So the first read asks for
// data.promotion.coupon.applies_to; if Stripe refuses that expansion (4xx),
// the second asks for data.promotion.coupon only, and every code's SCOPE is
// then unknown (reported, never assumed). A coupon that stays an id string has
// an unknown DISCOUNT too.
export async function listStripePromotionCodes(opts = {}) {
  const attempts = [
    { expand: "data.promotion.coupon.applies_to", scopeRead: true },
    { expand: "data.promotion.coupon", scopeRead: false },
  ];
  let r = null;
  for (const a of attempts) {
    r = await listAll(opts, STRIPE_PROMOS_URL, [["active", "true"], ["expand[]", a.expand]],
      "no Stripe read key configured");
    if (r.readable) {
      return { ...r, scope_expanded: a.scopeRead, items: r.items.map((pc) => promoItem(pc, a.scopeRead)) };
    }
    if (!(r.status >= 400 && r.status < 500 && r.status !== 401 && r.status !== 429)) break;
  }
  return r;
}
function promoItem(pc, scopeRead) {
  ...
  const pct = c && typeof c.percent_off === "number" ? c.percent_off : null;
  const amt = c && typeof c.amount_off === "number" ? c.amount_off : null;
  const discount = c && (pct !== null || amt !== null)
    ? { percent_off: pct, amount_off: amt, currency: String(c.currency || "").toLowerCase() } : null;
  let scope = null;
  if (c && scopeRead) {
    const at = c.applies_to;
    if (at === undefined || at === null) scope = { all: true };
    else if (at && Array.isArray(at.products)) scope = { all: false, products: at.products.map(String) };
  }
  return { id: String((pc && pc.id) || ""), active: !!(pc && pc.active), discount, scope };
}

// GET /v1/payment_links?active=true&limit=100 with line items expanded. If the
// expansion is refused (4xx), the links are listed without it and at most
// MAX_LINE_ITEM_READS GET /v1/payment_links/{id}/line_items fill in the price
// ids; a link beyond that has prices: null (unknown).
export async function listStripePaymentLinks(opts = {}) {
  const first = await listAll(opts, STRIPE_LINKS_URL, [["active", "true"], ["expand[]", "data.line_items"]],
    "no Stripe read key configured");
  const priceIds = (list) => (list && Array.isArray(list.data)
    ? list.data.map((li) => String((li && li.price && li.price.id) || "")).filter(Boolean) : null);
  if (first.readable) {
    return { ...first, items: first.items.map((l) => ({ id: String((l && l.id) || ""), active: !!(l && l.active),
      prices: priceIds(l && l.line_items) })) };
  }
  if (!(first.status >= 400 && first.status < 500 && first.status !== 401 && first.status !== 429)) return first;
  const bare = await listAll(opts, STRIPE_LINKS_URL, [["active", "true"]], "no Stripe read key configured");
  if (!bare.readable) return bare;
  const items = [];
  let reads = 0;
  for (const l of bare.items) {
    const id = String((l && l.id) || "");
    let prices = null;
    if (/^[A-Za-z0-9_]+$/.test(id) && reads < MAX_LINE_ITEM_READS) {
      reads++;
      const li = await listAll({ ...opts, maxPages: 1 }, STRIPE_LINKS_URL + "/" + id + "/line_items", [],
        "no Stripe read key configured");
      prices = li.readable ? li.items.map((x) => String((x && x.price && x.price.id) || "")).filter(Boolean) : null;
    }
    items.push({ id, active: !!(l && l.active), prices });
  }
  return { ...bare, items, line_item_reads: reads, expanded: false };
}

// The generic GET pager behind every reader except subscriptions.
async function listAll(opts, base, params, noKeyDetail) {
  const fetchImpl = opts.fetchImpl || null;
  const key = opts.key || "";
  const version = opts.apiVersion || STRIPE_API_VERSION;
  const maxPages = opts.maxPages || MAX_PAGES;
  const bad = (reason, status, requestId, pages, detail) =>
    ({ readable: false, items: [], status, reason, requestId, pages, detail });
  if (!fetchImpl) return bad("no-fetch", 0, null, 0, "no fetch implementation available");
  if (!key) return bad("no-key", 0, null, 0, noKeyDetail);
  const items = [];
  ...
  while (pages < maxPages) {
    ...
    try {
      ...
      return bad("network", 0, requestId, pages, "network/TLS failure: " + String((e && e.message) || e).slice(0, 160));
    }
    const rid = hdr(resp, "Request-Id");
    if (rid) requestId = rid;
    if (!(resp.status >= 200 && resp.status < 300)) {
      const rl = hdr(resp, "Stripe-Rate-Limited-Reason");
      return bad(httpReason(resp.status, rl), resp.status, requestId, pages, httpDetail(resp.status, rl, requestId));
    }
    let body = null;
    try { body = await resp.json(); } catch (_) { body = null; }
    if (!body || body.object !== "list" || !Array.isArray(body.data) || typeof body.has_more !== "boolean") {
      return bad("bad-shape", resp.status, requestId, pages,
        "HTTP 200 but the body was not a Stripe list object with a boolean has_more");
    }
    pages++;
    for (const x of body.data) items.push(x);
    if (body.has_more === false) {
      return { readable: true, items, status: resp.status, reason: null, requestId, pages, detail: null };
    }
    const last = body.data[body.data.length - 1];
    if (!last || !last.id) {
      return bad("no-cursor", resp.status, requestId, pages, "has_more was true but the page carried no id to page from");
    }
    startingAfter = String(last.id);
  }
  return bad("page-cap", 200, requestId, pages, `stopped after ${maxPages} pages with has_more still true`);
}
...
async function pull(fetchImpl, key, version, expand, maxPages) {
  ...
  while (pages < maxPages) {
    ...
    let body = null;
    try { body = await resp.json(); } catch (_) { body = null; }
    // has_more must be a real boolean: a missing or string has_more ("false")
    // is not a completeness signal, so the read is bad-shape.
    if (!body || body.object !== "list" || !Array.isArray(body.data) || typeof body.has_more !== "boolean") {
...
// Which enabled endpoints deliver to stripe-webhook.js on siteHost.
//   exact: enabled endpoints whose host is siteHost and path is exactly
//          WEBHOOK_PATH (their events are what the handler receives).
//   other: enabled endpoints on siteHost (or www.siteHost) at any other path.
// Shared by reconcile() (churn coverage) and the rehearsal's C8 (a).
export function siteEndpoints(webhookResult, siteHost) {
  const eps = webhookResult && webhookResult.readable && Array.isArray(webhookResult.endpoints)
    ? webhookResult.endpoints : [];
  const enabled = eps.filter((e) => e && e.status !== "disabled");
  const exact = enabled.filter((e) => e.url_host === siteHost && e.url_path === WEBHOOK_PATH);
  const other = enabled.filter((e) => (e.url_host === siteHost || e.url_host === "www." + siteHost) &&
    !exact.includes(e));
  const events = new Set();
  for (const e of exact) for (const x of e.enabled_events || []) events.add(String(x));
  return { exact, other, events };
}
...
export async function reconcile(input) {
  ...
  // Every finding and notice carries a PRIVATE ref {row, sub}: row indexes
  // the roster array passed in, sub indexes stripeResult.subs (null when the
  // finding is about neither). The de-duplication key includes it, because
  // subject is domain-masked and two buyers on one mail provider share it.
  const rowIdx = new Map();
  roster.forEach((r, i) => { if (r && typeof r === "object" && !rowIdx.has(r)) rowIdx.set(r, i); });
  const rowOf = (r) => (rowIdx.has(r) ? rowIdx.get(r) : null);
  const subIdx = new Map();
  (Array.isArray(sr.subs) ? sr.subs : []).forEach((x, i) => { if (x && typeof x === "object" && !subIdx.has(x)) subIdx.set(x, i); });
  const subOf = (x) => (subIdx.has(x) ? subIdx.get(x) : null);
  const mkRef = (row, sub) => ({ row: typeof row === "number" ? row : null, sub: typeof sub === "number" ? sub : null });

  const seen = new Set();
  const push = (name, f, ref = mkRef(null, null)) => {
    const k = name + "|" + f.type + "|" + (f.subject || "") + "|" + ref.row + "|" + ref.sub;
    if (seen.has(k)) return;
    seen.add(k);
    if (C[name].findings.length < 50) C[name].findings.push({ ...f, ref });
  };
  const notices = [];
  const addNotice = (n, ref = mkRef(null, null)) => { if (notices.length < 50) notices.push({ ...n, ref }); };
  ...
  if (sendLogReadable && rosterReadable) {
    ...
    if (cur && prev) {
      ...
      for (const addr of prevOk) {
        ...
        local.dropped++;
        // Prefer an ACTIVE row with this address (it decides the notice), else
        // the first row with it; ref.row is that row's index, or null.
        const hits = roster.filter((r) => r && lc(r.email) === addr);
        const row = hits.find(isActiveRow) || hits[0] || null;
        const tref = mkRef(row ? rowOf(row) : null, null);
  ...
  // Judged by EXACT endpoint path (rehearsal port): only an enabled endpoint
  // at WEBHOOK_PATH on siteHost reaches stripe-webhook.js. An endpoint on the
  // host at another path may be a redirect, a proxy or a stale route; its
  // events are not counted, and the coverage is reported as ambiguous.
  let churnCoverage = { known: false, subscribed: null, endpoints_for_host: 0, wildcard: false };
  if (wh.readable) {
    const site = siteEndpoints(wh, siteHost);
    const mine = site.exact;
    churnCoverage.endpoints_for_host = mine.length;
    churnCoverage.wildcard = site.events.has("*");
    if (mine.length === 0 && site.other.length > 0) {
      push("extra_roster_rows", {
        type: "webhook_endpoint_path_ambiguous", severity: "blocking", subject: siteHost,
        detail: "enabled webhook endpoint(s) exist on " + siteHost + " but none at exactly " +
                WEBHOOK_PATH + ", so whether any event reaches the handler cannot be read from Stripe.",
      });
    } else {
      churnCoverage.known = true;
      churnCoverage.subscribed = site.events.has("*") || site.events.has(CHURN_EVENT);
    }
    if (mine.length === 0 && site.other.length === 0) {
      push("extra_roster_rows", {
  ...
  // Two or more Stripe customers entitled under ONE email (rehearsal port).
  // Only visible on an expanded read. A re-subscription arrives as a new
  // customer id; if the old one still holds a live subscription the person is
  // billed twice, and a cancellation of either id deactivates the one roster
  // row whichever way the match goes.
  for (const [e, subsForEmail] of entitledByEmail) {
    const ids = new Set(subsForEmail.map(subCustomerId).filter(Boolean));
    if (ids.size < 2) continue;
    const hit = roster.find((r) => r && lc(r.email) === e && isActiveRow(r)) || roster.find((r) => r && lc(r.email) === e);
    push("identity_conflicts", {
      type: "duplicate_stripe_customers_same_email", severity: "amber", subject: maskEmail(e),
      distinct_customer_ids: ids.size,
      detail: ids.size + " different Stripe customers hold a live subscription under this email.",
    }, mkRef(hit ? rowOf(hit) : null, subOf(subsForEmail[0])));
  }
  ...
  for (const s of entitled) {
    ...
    const si = subOf(s);
    if (byId.length) {
      if (byId.some(isActiveRow)) { trialNotice(s, tr, cemail, addNotice, mkRef(rowOf(byId.find(isActiveRow)), si)); continue; }
      push("missing_payers", {
    ...
    // Two literal types (rather than one ternary) so every `type:` in this
    // file is a string literal the rehearsal's RECONCILE_MAP can be checked
    // against.
    if (cl.kind === "mass") {
      push("missing_payers", {
        type: "payer_missing_from_roster",
      ...
      push("missing_payers", {
        type: "unclassified_payer_missing_from_roster",
        severity: "red", subject: cemail ? maskEmail(cemail) : "customer " + tag,
        stripe_id_tag: tag, stripe_status: s.status, cents, trialing: tr.trialing,
        matched_on: cl.matched_on,
        detail: "a live subscription of " + cents + "c with no roster row, and its price id is in no " +
  ...
  // THE INVARIANT (rehearsal port): every entitled subscription is either
  // considered in direction A or classified as the sibling ("other"). If the
  // two ever disagree, a payer was skipped without being counted anywhere.
  if (entitled.length !== considered + classifiedOther) {
    push("read_integrity", {
      type: "invariant_broken", severity: "blocking", subject: null,
      detail: "stripe_entitled_total !== considered + classified_other",
    });
  }
  ...
  for (const r of activeRows) {
    ...
    if (cid) {
      ...
      if (live.length) {
        for (const s of live) periodNotices(s, r, addNotice, mkRef(ri, subOf(s)));
        continue;
      }
...
function trialNotice(sub, tr, cemail, addNotice, ref) {
...
function periodNotices(sub, row, addNotice, ref) {
  ...
  if (tr.trialing) trialNotice(sub, tr, null, addNotice, ref);
}
